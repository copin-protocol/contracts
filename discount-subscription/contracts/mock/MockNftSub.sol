...
contract MockNftSub is ISubscriptionNFT, ERC721  {
    ...
    function _initializeTier(uint256 tierId, string memory name, uint256 price, bool enabled) private {
        tierNames[tierId] = bytes32(bytes(name));
        tierPrices[tierId] = price;
        tierEnabled[tierId] = enabled;
    }

    function tiers(uint256 tierId) external view override returns (
        bytes32 name,
        uint256 price,
        uint256 quantity,
        bool enabled
    ...
    function mint(uint256 tierId, uint256 duration) external payable override {
        ...
        uint256 fee = (tierPrices[tierId] * duration * (100 - duration + 1)) / 100;
